...
public partial class SocketGame : MonoBehaviour {
    public string address = "daqingmj.youhao88.com";
...
/////////////////////////////////
   ...
   /*             if (hc.handCards[0] == -1)
    ...
    //这里给服务器发送杠消息
    public void DoGang(int card)
    {
        if (MJUtils.MinGang())
        {
            DoGameOperPlayerActionSyn(MJUtils.ACT_MIN_GANG, card);
        }
        else if (MJUtils.BuGang())
        {
            DoGameOperPlayerActionSyn(MJUtils.ACT_BU_GANG, card);
        }
        else if (MJUtils.AnGang())
        {
