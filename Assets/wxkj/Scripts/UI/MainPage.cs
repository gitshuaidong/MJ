...
public class MainPage : MainPageBase
{
    ...
    public override void InitializeScene()
    {
        ...
        detail.NormalRoomButton_Button.onClick.AddListener(OnClickSingle);
        detail.SingleButton_Button.onClick.AddListener(OnClickNormal);
    ...
    IEnumerator OnLoadImage()
    {
        WWW w;
        yield return w = new WWW("http://daqingmj.youhao88.com:8081/award.png");
 
        if (w.isDone&&w.error==null)
        {
            print("资源加载完成");
            Texture2D tex2d = w.texture;
            Sprite _sprite = Sprite.Create(tex2d, new Rect(0, 0, tex2d.width, tex2d.height), new Vector2(0, 0));
            detail.NormalRoomButton_Image.sprite = _sprite;
        }else
        {
            print("资源加载失败");
        }
    ...
    private void OnClickShare()
    {
        ...
        MyShareSDK.MyshareSDK.ShareWeChatMoments();
    }
    ...
    void Update_Marquee()
    {
        if (!marquee)
        {
            if (Game.Instance != null && Game.Instance.marquee.Count > 0)
            {
                marquee = true;
